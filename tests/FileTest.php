...
final class FileTest extends AbstractFilesystemTestCase
{
    ...
    public function putContents(AdapterInterface $filesystem): void
    {
        ...
        $writtenLength = await($filesystem->detect($fileName)->then(static fn (NotExistInterface $notExist): PromiseInterface => $notExist->createFile())->then(static function (FileInterface $file) use ($fileContents): PromiseInterface {
    ...
    public function runMultipleFilesTests(AdapterInterface $filesystem, int $fileCount, int $fileSize, int $chunkCount): void
    {
        ...
        foreach ($fileContents as $fileName => $fileContent) {
            ...
            $promises[$fileName] = $filesystem->detect($fileName)->then(static function (FileInterface $file) use ($queue): PromiseInterface {
                return new Promise(static function (callable $resolve, callable $reject) use ($queue, $file): void {
                    $bytesWritten  = 0;
                    $writeFunction = static function () use (&$writeFunction, &$bytesWritten, $queue, $file, $resolve, $reject): void {
                        if ($queue->count() > 0) {
                            $file->putContents($queue->dequeue(), FILE_APPEND)->then(static function (int $writtenBytes) use (&$writeFunction, &$bytesWritten): void {
